...
/// AES-CTR keystream XOR
///
/// PointyCastle's stream ciphers process one byte per call.
/// This is too slow, XOR whole chunks instead
class AesCtrCipher {
  AesCtrCipher(Uint8List key, Uint8List iv)
      : _engine = AESEngine()..init(true, KeyParameter(key)),
        _counter = Uint8List.fromList(iv),
        _keystream = Uint8List(16),
        _keystreamOffset = 16;

  final AESEngine _engine;
  final Uint8List _counter;
  final Uint8List _keystream;
  int _keystreamOffset;

  void process(Uint8List input, int length, Uint8List output) {
    var offset = 0;
    while (offset < length) {
      ...
      final available = _keystream.length - _keystreamOffset;
      final take = length - offset < available ? length - offset : available;
      for (var i = 0; i < take; i++) {
        output[offset + i] =
            input[offset + i] ^ _keystream[_keystreamOffset + i];
      }
      offset += take;
